...
| 阶段 | 必须交付什么 | 禁止 |
|------|----------------|------|
| **沉淀（模式 1）** | 可反复出片的 **课型引擎包**（或挂到已有引擎）+ 内容契约 + 样例内容 + 样例图 + **关键帧保真** | 只存 markdown 页序、只出通用壳 PPT、只抄框架不对照参考 |
| **复用（模式 2）** | 主题内容写满 → **open 审** → **同一引擎**出片 → **open PPT+草稿** | 换主题时换皮、用 `build_pptx.py` 冒充签样课型、只丢路径不打开 |

**业务体验默认（v0.3.4+）：** 不问 1/2；结果只写 `workspace/`；完成后默认 `open` 内容与 PPT。
...
- [ ] 样例 pptx **本机能打开**，且已对业务 **默认 open** 内容+PPT  
- [ ] change-list 说明换主题只换内容/图  
- [ ] 包装无假图；敏感表述可标待审  
- [ ] 关键帧过 `references/fidelity-qa-checklist.md`（至少 path-only；声称像金样须 gold-aligned）  
- [ ] 产物只在 `workspace/`，未改 `skills/`  

### 为何 WorkBuddy 容易只复刻「框架」（代理必读）

| 原因 | 正确做法 |
|------|----------|
| 把「复刻」做成页序/大纲 markdown | 必须引擎布局代码 + 写满 JSON + 出片 |
| 走 `build_pptx.py` 或通用 PPT skill | 只用 `engines/*` 签样路径 |
| 未读 `FIDELITY.md` / HANDOVER / 金样可编辑 PPT | 逐页差分字阶、runs、拓扑、插图 fit |
| 会话偷懒只交 structure/ | 交付前对照参考打开 PPT 并排看 |
| 本 Skill 未装全 / 未 `npm i` 引擎 | 安装后先跑通 gold 样例再沉淀 |

业务只需要记住：

> **存模板 = 存成以后能自动出片、观感跟你参考同一量级的那一套；换主题 = 换内容再按同一套出片。**  
> 你不用选编号；做好的文件会直接打开给你看。
